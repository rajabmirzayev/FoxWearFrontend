...
export default function QuickViewModal({ product, onClose, onLike, isLiked }: QuickViewModalProps) {
  ...
  const handleAddToCart = async () => {
    ...
    setIsAdding(true);
    try {
      // Find the specific item ID for the selected size in the selected color
      const selectedItem = product?.colors[selectedColorIndex].items.find(
        item => item.productSize.id === selectedSizeId
      );
      
      if (selectedItem) {
        await addToCart(selectedItem.id, 1);
        // Trigger fly animation
        const currentImage = product?.colors[selectedColorIndex]?.images?.[selectedImageIndex]?.image;
        if (currentImage) {
          setShowFlyItem(currentImage);
              ...
              <div className="w-full md:w-1/2 p-10 flex flex-col justify-center space-y-6">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1 text-primary">
                      {[1, 2, 3, 4, 5].map((star) => {
                        const isFull = averageRate >= star;
                        const isHalf = !isFull && averageRate >= star - 0.5;
                        return (
                          <span key={star} className={`material-symbols-outlined text-xs ${isFull ? 'icon-fill' : ''}`}>
                ...
                <div className="flex gap-3">
                  <button 
                    ...
                    bg-primary text-white dark:text-background-dark border border-primary/10 hover:bg-white hover:text-primary
                    dark:bg-white dark:hover:bg-background-light dark:hover:text-white disabled:opacity-50">
                    {isAdding ? 'Adding...' : 'Add to Cart'}
                  </button>
                  <button 
                    onClick={(e) => onLike(product.id, e)}
                    className={`px-5 border-[0.5px] border-primary/10 transition-all duration-300 flex items-center justify-center group/btn cursor-pointer
